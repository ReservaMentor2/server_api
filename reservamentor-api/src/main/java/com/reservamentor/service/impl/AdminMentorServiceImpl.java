...
public class AdminMentorServiceImpl implements AdminMentorService {
    ...
    public List<InformacionMentorDTO> getMentorsByDia(String dia) {
        ...
        if (mentors.isEmpty()) {
            throw new ResourceNotFoundException("No se encontraron mentores disponibles para el día: " + dia);
        }
        return mentors.stream()
                .map(mentorMapper::IMtoDTO)
                .toList();
    ...
    @Transactional(readOnly = true)
    @Override
    public List<InformacionMentorDTO> getMentorsByHora(LocalTime horaInicio, LocalTime horaFin) {
        ...
        if (mentors.isEmpty()) {
            throw new ResourceNotFoundException("No se encontraron mentores disponibles entre las horas: " + horaInicio + " y " + horaFin);
        }
        return mentors.stream()
                .map(mentorMapper::IMtoDTO)
                .toList();
    ...
    @Transactional(readOnly = true)
    @Override
    public List<InformacionMentorDTO> getMentorsByDiaAndHora(String dia, LocalTime horaInicio, LocalTime horaFin) {
        ...
        if (mentors.isEmpty()) {
            throw new ResourceNotFoundException("No se encontraron mentores disponibles para el día: " + dia + " entre las horas: " + horaInicio + " y " + horaFin);
        }
    ...
    @Transactional
    @Override
    public InformacionMentorDTO update(Integer id, InformacionMentorDTO updateInformacionMentorDTO) {
        Mentor mentorFromDb = mentorRepository.findById(id)
                .orElseThrow(() -> new RuntimeException("El mentor con ID " +id+ " no fue encontrado"));
